...
from sklearn.model_selection import GroupKFold, KFold,RepeatedStratifiedKFold
...
Fold = RepeatedStratifiedKFold(n_splits=CONFIG['n_fold'],n_repeats=3 ,random_state=CONFIG['seed'])#StratifiedGroupKFold
...
if __name__ == '__main__':
    ...
    for fold in range(0, CONFIG['n_fold']*3):
        print(f"{y_}====== Fold: {fold} ======{sr_}")
        run = wandb.init(project='FeedBack2',
                         config=CONFIG,
                         job_type='Train',
                         group=CONFIG['group'],
