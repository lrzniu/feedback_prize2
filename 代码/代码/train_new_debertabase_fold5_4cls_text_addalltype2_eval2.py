...
def train_one_epoch(model, optimizer, scheduler, dataloader, device, epoch,eval_step):#
    model.train()
    epoch_loss=2
    dataset_size = 0
    running_loss = 0.0
...
def train_one_epoch_and_validn(model, optimizer, scheduler, train_dataloader, vaild_dataloader,device, epoch,eval_step):#
    model.train()
    epoch_loss=2
    dataset_size = 0
    running_loss = 0.0
    ...
    step_num=length//2
    start_step=step_num*eval_step
    end_step = start_step+step_num
    bar = tqdm(enumerate(train_dataloader), total=length)
    for step, data in bar:
        if step>=start_step and step<end_step:
...
def run_training(model, optimizer, scheduler, device, num_epochs, fold):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        train_epoch_loss = train_one_epoch(model, optimizer, scheduler,
                                           ...
                                           device=CONFIG['device'], epoch=epoch,eval_step=0)
        ...
        train_epoch_loss = train_one_epoch(model, optimizer, scheduler,
                                           dataloader=train_loader,
                                           device=CONFIG['device'], epoch=epoch,eval_step=1)
