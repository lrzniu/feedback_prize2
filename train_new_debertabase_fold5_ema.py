...
def train_one_epoch(model, optimizer, scheduler,ema, dataloader, device, epoch):
...
@torch.no_grad()
def valid_one_epoch(model, dataloader,ema,device, epoch):
    ema.apply_shadow()
    model.eval()
...
def run_training(model, optimizer, scheduler, device, num_epochs, fold):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        ema.register()
        gc.collect()
        train_epoch_loss = train_one_epoch(model, optimizer, scheduler, ema,
                                           dataloader=train_loader,
                                           device=CONFIG['device'], epoch=epoch)

        val_epoch_loss = valid_one_epoch(model, valid_loader,ema, device=CONFIG['device'],
                                         epoch=epoch)
