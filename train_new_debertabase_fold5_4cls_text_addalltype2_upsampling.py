...
df1 = df[df.discourse_effectiveness == 0].reset_index(drop=True)
df2 = df[df.discourse_effectiveness== 1].reset_index(drop=True)
df3 = df[df.discourse_effectiveness == 2].reset_index(drop=True)
...
def prepare_loaders(fold):
    ...
    for i in range(df_train.shape[0]):
        if df_train["discourse_effectiveness"][i] != 0:
            df2 = df2.append(df_train.loc[[i]], ignore_index=True)
