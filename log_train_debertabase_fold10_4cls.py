...
df = pd.read_csv("HDFS_log_structured_label.csv")
df_label=pd.read_csv("anomaly_label.csv")
# df["label"]=2
# df["Content2"] = 0
# for i in range(df.shape[0]):
#     a= re.findall(r'(blk_-?\d+)', df['Content'][i])
#     df["Content2"][i] =a[0]
#
# df.to_csv("HDFS_log_structured_label.csv",index=False)

df_sub=df.sample(n=100000, random_state=None)
df_id=df_sub.set_index("Content2")
dfcat=[]
for i in range(df_label.shape[0]):
    df_id["label"].loc[df_label["BlockId"][i]]=df_label["Label"][i]
    dfcat.append(df_id.loc[df_label["BlockId"][i]])
df2=pd.concat(dfcat)
df2=df2.reset_index()
df2.to_csv("HDFS_log_structured_label2.csv",index=False)
...
print(df["blockid"][0])
# for i in range(df.shape[0]):
#     df["blockid"][i]=re.findall(r'(blk_-?\d+)', df['Content'][i])
#     if
