...
def train_one_epoch(model, optimizer, scheduler, dataloader, device, epoch):
    ...
    for step, data in bar:
        ids = data['input_ids'].to(args.local_rank, dtype=torch.long)
        mask = data['attention_mask'].to(args.local_rank, dtype=torch.long)
        targets = data['target'].to(args.local_rank, dtype=torch.long)
...
def prepare_loaders(fold):
    ...
    train_loader = DataLoader(train_dataset, batch_size=CONFIG['train_batch_size'], collate_fn=collate_fn,
                              num_workers=2, shuffle=False, pin_memory=True, drop_last=True,sampler=train_sampler)
    valid_loader = DataLoader(valid_dataset, batch_size=CONFIG['valid_batch_size'], collate_fn=collate_fn,
                              num_workers=2, shuffle=False, pin_memory=True,sampler=valid_sampler)
...
if __name__ == '__main__':
    ...
    for fold in range(0, CONFIG['n_fold']):
        ...
        model = DDP(model.cuda(), device_ids=[args.local_rank],find_unused_parameters=True)
